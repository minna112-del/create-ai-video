...
const AttendanceOffice = {
  ...
  async checkIn(){
    ...
    try{await FB.setDoc(FB.doc(FB.db,'attendance_records',`${this.today}_${uid}`),{uid,date:this.today,employeeName:staff.name||FB.auth.currentUser?.email||'Staff',employeeId:staff.employeeId||'',branchName:staff.branchName||'Zone-A',shift:staff.shift||'General',shiftStart,shiftHours:+staff.shiftHours||8,status:late?'late':'present',checkInAt:FB.serverTimestamp(),checkInLocation:loc,source:'web',createdAt:FB.serverTimestamp(),updatedAt:FB.serverTimestamp()},{merge:true});toast(late?'চেক-ইন হয়েছে—দেরি হিসেবে রেকর্ড হয়েছে':'✓ চেক-ইন সফল','success');await this.refresh();}catch(e){toast('চেক-ইন হয়নি: '+e.message,'error');}
  },
